...
import { genTokenIdForMainStore, signPutDataOnSaleCreated } from 'blockchain/utils-created';
import { Modal } from '@mui/material';
import FormCreate from 'components/templates/create-nft/ERC1155';
...
import { NFT_SALE_TYPES, MARKET_RAW_FEE_BUY_TOKEN } from 'constants/index';
...
const CreateSingle = () => {
  ...
  const [txHashFlow, setTxhashFlow] = useState<any>({
    initWallet: '',
    approveUMAD: '',
    approveCollection: '',
    confirmForSign: '',
  ...
  const { values, errors, touched, setFieldValue, setFieldError, handleSubmit, setFieldTouched } = formik;
  ...
  useEffect(() => {
    ...
    if (result !== conditionAssetDetails) setConditionAssetDetails(Boolean(result));
  }, [errors, values, stepCreate, values.nftImagePreview]);

  //step Advanced Details
  useEffect(() => {
    const result =
      (errors.unlockableContent && values.isUnlockableContent) ||
      !_.isEmpty(errors.properties) ||
      !_.isEmpty(errors.levels) ||
      !_.isEmpty(errors.stats) ||
      errors.nftImagePreview ||
      (values.nftAudio && errors.nftAudio) ||
      (values.nftVideo && errors.nftVideo);

    if (result !== conditionAdvancedDetails) setConditionAdvancedDetails(Boolean(result));
  ...
  const handleValidateUpload = () => {
    ...
    if (values.nftAudio && values.nftAudio) {
  ...
  const handleNextStep = () => {
    ...
    if (stepCreate === CREATE_ASSET_TYPE.ASSET_DETAIL && conditionAssetDetails) {
      ...
      if (!values.supply) {
        setFieldError('supply', 'Supply is not allowed to be empty.');
    ...
    if (
      ...
      if (!values.quantity) {
        setFieldError('quantity', 'Quantity is not allowed to be empty.');
  ...
  const handleCreateNFT = async (values: any) => {
    ...
    if (unlockableContent) params.unlockableContent = unlockableContent.trim();
  ...
  const getCollectionOfOwner = async () => {
    const [response, error] = await collectionService.getCollectionOfOwner(TYPE_COLLECTION.ERC1155);
    if (response) {
      ...
      <div className="flex gap-x-24 flex-col-reverse lg:flex-row">
        ...
        <div className="lg:mt-[-87px]">
          <UploadMedia
            ...
            changeHandlerNftPreview={(event: any) => changeHandlerNftPreview(event, setFieldValue, setFieldTouched)}
            formik={formik}
          />
        </div>
        <Modal
        ...
        {modalFollowCreateNFT && (
          <ModalSellCreatedNFT
            ...
            handleClose={() => setModalFollowCreateNFT(false)}
            values={values}
