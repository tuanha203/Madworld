...
const VideoPlayer = (props: VideoProps) => {
  ...
  let convertSrc = getBaseURLFromIPFS(src);
  let convertImageUrl = getBaseURLFromIPFS(imageUrl);

  // const duration = useMemo(() => videoRef?.current?.duration || 0, [videoRef, videoStatus])

  // const currentTime = useMemo(() => videoRef?.current?.currentTime || 0, [videoRef, videoStatus])
  ...
  return (
    ...
    >
      <video
        ref={videoRef}
        className="video-player_video"
        onClick={tooglePlay}
        autoPlay
        muted={currentVolume === 0}
        key={convertSrc || convertImageUrl}
        style={{
          background: `no-repeat url(${convertImageUrl})`,
          backgroundSize: 'contain',
          backgroundRepeat: 'no-repeat',
          backgroundPosition: 'center',
        }}
      >
        <source src={convertSrc} />
      </video>
      <div
